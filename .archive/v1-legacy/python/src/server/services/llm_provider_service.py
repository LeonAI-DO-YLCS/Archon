...
async def get_llm_client(
    ...
    try:
        ...
        if provider_name == "openai":
            ...
            # Start with explicit base_url if provided
            lmstudio_url = base_url
            
            # If not provided, try to get from settings
            if not lmstudio_url:
                try:
                    # We might have cached settings available
                    if 'rag_settings' in locals() and rag_settings:
                         lmstudio_url = rag_settings.get("LMSTUDIO_BASE_URL")
                    
                    # If not, try to fetch
                    if not lmstudio_url:
                         rag_s = await credential_service.get_credentials_by_category("rag_strategy")
                         lmstudio_url = rag_s.get("LMSTUDIO_BASE_URL")
                except Exception:
                    pass
            ...
            if not lmstudio_url.endswith('/v1'):
                lmstudio_url = f"{lmstudio_url}/v1"

            client = openai.AsyncOpenAI(
                api_key="lm-studio",
                base_url=lmstudio_url,
            )
            logger.info(f"LMStudio client created successfully with base URL: {lmstudio_url}")
...
async def get_embedding_model(provider: str | None = None) -> str:
    ...
    try:
        ...
        if provider_name == "openai":
            ...
            # LMStudio usually hosts models like nomic-embed-text
...
def is_valid_embedding_model_for_provider(model: str, provider: str) -> bool:
    ...
    if provider_lower == "openai":
        ...
        # LMStudio can run any model, so we're permissive
        return True
    else:
