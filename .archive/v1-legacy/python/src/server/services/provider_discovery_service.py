...
class ProviderDiscoveryService:
    ...
    async def discover_lmstudio_models(self, base_url: str) -> list[ModelSpec]:
        """Discover available LMStudio models."""
        cache_key = f"lmstudio_models_{base_url}"
        cached = self._get_cached_result(cache_key)
        if cached:
            return cached
        ...
        try:
            ...
            # Use OpenAI-compatible client pattern
            # Using a dummy API key as LMStudio typically ignores it or accepts 'lm-studio'
            import openai
            client = openai.AsyncOpenAI(base_url=api_url, api_key="lm-studio")
            
            response = await client.models.list()

            for model in response.data:
                # In LMStudio, getting detailed specs is harder via just API
                # We'll use reasonable defaults and try to infer from name
                name = model.id
                context_window = 4096 # Default
                
                # Heuristics for context window
                ...
                # Vision support deduction
                supports_vision = any(p in lower_name for p in VISION_MODEL_PATTERNS)
                
                # Embedding support deduction
                supports_embeddings = any(p in lower_name for p in EMBEDDING_MODEL_PATTERNS)
                
                models.append(ModelSpec(
                    name=name,
                    provider="lmstudio",
                    context_window=context_window,
                    supports_tools=True, # LMStudio often supports tools depending on model, optimistic default
                    supports_vision=supports_vision,
                    supports_embeddings=supports_embeddings,
                    description=f"Local model via LMStudio: {name}"
                ))

            self._cache_result(cache_key, models)
            logger.info(f"Discovered {len(models)} LMStudio models from {base_url}")

        except Exception as e:
            logger.error(f"Error discovering LMStudio models from {base_url}: {e}")
    ...
    async def check_provider_health(self, provider: str, config: dict[str, Any]) -> ProviderStatus:
        ...
        try:
            if provider == "openai":
                ...
                base_url = config.get("base_url", DEFAULT_LMSTUDIO_URL)
    ...
    async def get_all_available_models(self) -> dict[str, list[ModelSpec]]:
        ...
        try:
            ...
            # LMStudio
            lmstudio_url = rag_settings.get("LMSTUDIO_BASE_URL", DEFAULT_LMSTUDIO_URL)
            providers["lmstudio"] = await self.discover_lmstudio_models(lmstudio_url)
