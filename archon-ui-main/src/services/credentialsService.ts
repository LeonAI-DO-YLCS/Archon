...
class CredentialsService {
  ...
  // LMStudio Config
  async getLMStudioConfig(): Promise<{ baseUrl: string }> {
    const settings = await this.getRagSettings();
    return {
      baseUrl: settings.LMSTUDIO_BASE_URL || "http://host.docker.internal:1234/v1"
    };
  }

  async saveLMStudioConfig(config: { baseUrl: string }): Promise<void> {
