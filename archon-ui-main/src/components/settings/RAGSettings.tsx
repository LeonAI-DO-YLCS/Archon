...
const EMBEDDING_CAPABLE_PROVIDERS: ProviderKey[] = ['openai', 'google', 'openrouter', 'ollama', 'lmstudio'];
...
export const RAGSettings = ({
  ...
  const fetchLMStudioModels = useCallback(async () => {
    if (!ragSettings.LMSTUDIO_BASE_URL) return;
    
    try {
      setLoadingLMStudioModels(true);
      const response = await lmstudioService.discoverModels(ragSettings.LMSTUDIO_BASE_URL);
      setLmStudioModels(response.models);
    } catch (error) {
      console.error('Failed to fetch LM Studio models:', error);
      // Don't clear models on error to avoid flickering if transient
    } finally {
      setLoadingLMStudioModels(false);
    }
  }, [ragSettings.LMSTUDIO_BASE_URL]);

  // Effect to fetch LM Studio models when provider is selected or URL changes
  useEffect(() => {
    if (chatProvider === 'lmstudio' || embeddingProvider === 'lmstudio') {
      fetchLMStudioModels();
    }
  }, [chatProvider, embeddingProvider, fetchLMStudioModels]);
        ...
        <div className="mb-6">
          ...
          <div className={`grid gap-3 mb-4 ${
            ...
            {[
              ...
              { key: 'lmstudio', name: 'LM Studio', logo: '/lmstudio-logo.svg', color: 'indigo' }
            ]
              .filter(provider =>
          ...
          <div className="flex justify-between items-end">
            ...
            <div className="flex-1 max-w-md">
              {activeSelection === 'chat' ? (
                chatProvider === 'lmstudio' && lmStudioModels.length > 0 ? (
                  <Select
                    label="Chat Model"
                    value={getDisplayedChatModel(ragSettings)}
                    options={lmStudioModels.map(m => ({ value: m.name, label: m.name }))}
                    onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setRagSettings({
                      ...ragSettings,
                ...
                embeddingProvider === 'lmstudio' && lmStudioModels.filter(m => m.supports_embeddings).length > 0 ? (
                  <Select
                    label="Embedding Model"
                    value={getDisplayedEmbeddingModel(ragSettings)}
                    options={lmStudioModels.filter(m => m.supports_embeddings).map(m => ({ value: m.name, label: m.name }))}
                    onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setRagSettings({
                      ...ragSettings,
                      EMBEDDING_MODEL: e.target.value
                    })}
                    placeholder="Select LM Studio embedding model"
                  />
                ) : (
          ...
          {((activeSelection === 'chat' && chatProvider === 'lmstudio') || 
            ...
            <div className="mt-4">
              <LMStudioConfiguration onConfigChange={() => {
                reloadApiCredentials();
                // Also refresh models
                fetchLMStudioModels();
              }} />
