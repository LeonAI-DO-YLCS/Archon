...
const mockLoadConfig = mock(async () => ({
  ...
  assistants: {
    ...
    lmstudio: { enabled: false, baseUrl: 'http://lmstudio.local', defaultModel: 'qwen' },
...
describe('GET /api/config', () => {
  ...
  test('includes experimental provider settings without exposing LiteLLM secrets', async () => {
    mockLoadConfig.mockImplementationOnce(async () => ({
      assistant: 'claude',
      botName: 'Archon',
      assistants: {
    ...
    const body = (await response.json()) as {
      config: {
        assistants: {
          ...
          litellm: { enabled: boolean; baseUrl: string; defaultModel: string };
        };
      };
    };

    expect(body.config.assistants.ollama).toEqual({
      enabled: true,
      baseUrl: 'http://ollama.local',
      defaultModel: 'llama3.2',
    });
    expect(body.config.assistants.litellm).toEqual({
      enabled: true,
      baseUrl: 'http://litellm.local',
      defaultModel: 'gpt-4o-mini',
    });
    expect(JSON.stringify(body)).not.toContain('LITELLM_API_KEY');
