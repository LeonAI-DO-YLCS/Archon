...
const mockUpdateCodebaseAssistantType = mock(async (_id: string, _v: string) => {});
mock.module('@archon/core/db/codebases', () => ({
  listCodebases: mockListCodebases,
  getCodebase: mockGetCodebase,
  deleteCodebase: mockDeleteCodebase,
  updateCodebaseAllowEnvKeys: mockUpdateCodebaseAllowEnvKeys,
  updateCodebaseAssistantType: mockUpdateCodebaseAssistantType,
}));
...
describe('PATCH /api/codebases/:id', () => {
  beforeEach(() => {
    ...
    mockUpdateCodebaseAssistantType.mockReset();
  });
  ...
  test('updates aiAssistantType for future conversations only', async () => {
    mockGetCodebase
      .mockImplementationOnce(async () => MOCK_CODEBASE)
      .mockImplementationOnce(async () => ({ ...MOCK_CODEBASE, ai_assistant_type: 'ollama' }));

    const app = makeApp();
    const response = await app.request('/api/codebases/codebase-uuid-1', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ aiAssistantType: 'ollama' }),
    });

    expect(response.status).toBe(200);
    const body = (await response.json()) as { ai_assistant_type: string };
    expect(body.ai_assistant_type).toBe('ollama');
    expect(mockUpdateCodebaseAssistantType).toHaveBeenCalledWith('codebase-uuid-1', 'ollama');
  });

  test('rejects invalid aiAssistantType values', async () => {
    const app = makeApp();
    const response = await app.request('/api/codebases/codebase-uuid-1', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ aiAssistantType: 'invalid-provider' }),
    });
