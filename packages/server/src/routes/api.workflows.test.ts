...
describe('POST /api/workflows/validate', () => {
  ...
  test('returns unsupported-provider errors for experimental workflow providers', async () => {
    const app = createTestApp();
    registerApiRoutes(app, {} as WebAdapter, {} as ConversationLockManager);
    ...
    mockParseWorkflow.mockReturnValueOnce({
      ...
      error: {
        filename: 'test.yaml',
        error: "Unsupported workflow provider 'ollama'. Allowed providers: claude, codex.",
    ...
    const response = await app.request('/api/workflows/validate', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        definition: { name: 'my-workflow', description: 'test', provider: 'ollama', nodes: [] },
      }),
    });
    ...
    const body = (await response.json()) as { valid: boolean; errors: string[] };
    expect(body.valid).toBe(false);
    expect(body.errors[0]).toContain('Unsupported workflow provider');
  });
});
