...
function assertSupportedWorkflowProvider(provider: string): asserts provider is 'claude' | 'codex' {
  if (provider !== 'claude' && provider !== 'codex') {
    throw new Error(
      `Provider '${provider}' is unsupported for workflow execution. Allowed providers: claude, codex.`
    );
  }
}
...
async function resolveNodeProviderAndModel(
  ...
  if (node.provider) {
    assertSupportedWorkflowProvider(node.provider);
...
export async function executeDagWorkflow(
  ...
  assertSupportedWorkflowProvider(workflowProvider);
