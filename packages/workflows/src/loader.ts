...
function unsupportedProviderMessage(provider: string): string {
  return `Unsupported workflow provider '${provider}'. Allowed providers: claude, codex.`;
}
...
export function parseWorkflow(content: string, filename: string): ParseResult {
  try {
    ...
    if (raw.provider !== undefined && raw.provider !== 'claude' && raw.provider !== 'codex') {
      const providerValue =
        typeof raw.provider === 'string' ? raw.provider : '[invalid-provider-type]';
      return {
        workflow: null,
        error: {
          filename,
          error: unsupportedProviderMessage(providerValue),
          errorType: 'validation_error',
        },
      };
    }
