...
describe('validateWorkflowResources — provider boundary', () => {
  test('rejects unsupported workflow-level providers', async () => {
    const workflow = makeWorkflow('test', [
      { id: 'step1', command: 'assist' } as DagNode,
    ]) as WorkflowDefinition & {
      provider: string;
    };
    workflow.provider = 'ollama';
    ...
    expect(issues).toContainEqual(
      expect.objectContaining({
        field: 'provider',
        message: "Provider 'ollama' is unsupported for workflow execution",
      })
    );
  });

  test('rejects unsupported node-level providers', async () => {
    const workflow = makeWorkflow('test', [
      { id: 'step1', command: 'assist', provider: 'litellm' } as unknown as DagNode,
    ]);
    ...
    expect(issues).toContainEqual(
      expect.objectContaining({
        nodeId: 'step1',
        field: 'provider',
        message: "Provider 'litellm' is unsupported for workflow execution",
      })
    );
  });
});
