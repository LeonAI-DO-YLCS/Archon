...
function isSupportedWorkflowProvider(provider: string): provider is 'claude' | 'codex' {
  return provider === 'claude' || provider === 'codex';
}

function makeUnsupportedProviderIssue(provider: string, nodeId?: string): ValidationIssue {
  return {
    level: 'error',
    ...(nodeId ? { nodeId } : {}),
    field: 'provider',
    message: `Provider '${provider}' is unsupported for workflow execution`,
    hint: 'Use one of: claude, codex',
    suggestions: ['claude', 'codex'],
  };
}
...
export async function validateWorkflowResources(
  ...
  if (workflow.provider && !isSupportedWorkflowProvider(workflow.provider)) {
    issues.push(makeUnsupportedProviderIssue(String(workflow.provider)));
  }

  for (const node of workflow.nodes) {
    if ('provider' in node && node.provider && !isSupportedWorkflowProvider(node.provider)) {
      issues.push(makeUnsupportedProviderIssue(String(node.provider), node.id));
      continue;
    }
