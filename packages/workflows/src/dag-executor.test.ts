...
describe('executeDagWorkflow -- provider boundary', () => {
  it('rejects unsupported workflow providers before executing nodes', async () => {
    ...
    await expect(
      executeDagWorkflow(
        ...
        'conv-1',
        '/tmp',
        { name: 'invalid-provider', nodes: [node('step1')] },
        makeWorkflowRun(),
        'ollama' as never,
        undefined,
        '/tmp/artifacts',
    ...
    ).rejects.toThrow("Provider 'ollama' is unsupported for workflow execution");
  });
});
