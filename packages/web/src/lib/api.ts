...
export type ExperimentalProviderId = 'ollama' | 'lmstudio' | 'litellm';
...
export async function updateCodebase(
  id: string,
  input: { allowEnvKeys?: boolean; aiAssistantType?: CodebaseResponse['ai_assistant_type'] }
): Promise<CodebaseResponse> {
  return fetchJSON<CodebaseResponse>(`/api/codebases/${id}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
...
export async function validateProvider(
  ...
  return fetchJSON(`/api/providers/${provider}/validate`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(body),
  });
...
export async function listProviderModels(
  ...
  return fetchJSON(`/api/providers/${provider}/models`);
}
