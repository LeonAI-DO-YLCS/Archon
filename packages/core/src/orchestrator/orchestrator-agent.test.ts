...
const mockListReplayTranscript = mock(() => Promise.resolve([]));
mock.module('../db/messages', () => ({
  listReplayTranscript: mockListReplayTranscript,
}));
...
const mockGetAssistantType = mock(() => 'claude');
mock.module('../clients/factory', () => ({
  getAssistantClient: mock(() => ({
    sendQuery: mockSendQuery,
    getType: mockGetAssistantType,
  })),
  isExperimentalProvider: mock(
    (type: string) => type === 'ollama' || type === 'lmstudio' || type === 'litellm'
  ),
}));
...
describe('discoverAllWorkflows — remote sync', () => {
  beforeEach(() => {
    ...
    mockListReplayTranscript.mockReset();
    mockSendQuery.mockReset();
    mockGetAssistantType.mockReset();
    ...
    mockListReplayTranscript.mockImplementation(() => Promise.resolve([]));
    mockSendQuery.mockImplementation(async function* () {});
    mockGetAssistantType.mockImplementation(() => 'claude');
  });
...
describe('experimental provider transcript replay', () => {
  beforeEach(() => {
    mockGetOrCreateConversation.mockReset();
    mockGetCodebase.mockReset();
    mockListReplayTranscript.mockReset();
  ...
  test('replays persisted transcript only for experimental providers', async () => {
    mockGetOrCreateConversation.mockResolvedValueOnce(
      makeConversation({ ai_assistant_type: 'ollama' }) as unknown
    );
    mockListReplayTranscript.mockResolvedValueOnce([
      { role: 'user', content: 'Earlier question' },
      { role: 'assistant', content: 'Earlier answer' },
    ]);
    mockGetAssistantType.mockImplementation(() => 'ollama');
    mockSendQuery.mockImplementation(async function* () {
      yield { type: 'assistant', content: 'Fresh answer' };
      yield { type: 'result', stopReason: 'end_turn' };
    });

    const platform = makePlatform();
    await handleMessage(platform, 'conv-1', 'Newest question');

    expect(mockListReplayTranscript).toHaveBeenCalledWith('conv-1', 20);
  ...
  test('keeps native providers on assistant session continuity', async () => {
    mockGetOrCreateConversation.mockResolvedValueOnce(
      makeConversation({ ai_assistant_type: 'claude' }) as unknown
    );
    mockGetAssistantType.mockImplementation(() => 'claude');
    mockSendQuery.mockImplementation(async function* () {
      yield { type: 'assistant', content: 'Native answer' };
      yield { type: 'result', stopReason: 'end_turn', sessionId: 'claude-session-2' };
    });

    const platform = makePlatform();
    await handleMessage(platform, 'conv-1', 'Newest question');

    expect(mockListReplayTranscript).not.toHaveBeenCalled();
  });
});
