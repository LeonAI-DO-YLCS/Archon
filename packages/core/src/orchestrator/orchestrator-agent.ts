...
import { getAssistantClient, isExperimentalProvider } from '../clients/factory';
...
/** Max persisted transcript messages replayed to experimental providers */
const MAX_EXPERIMENTAL_TRANSCRIPT_MESSAGES = 20;
...
async function buildExperimentalTranscript(
  conversationId: string
): Promise<AssistantRequestOptions['transcript']> {
  const transcript = await messageDb.listReplayTranscript(
    conversationId,
    MAX_EXPERIMENTAL_TRANSCRIPT_MESSAGES
  );
...
export async function handleMessage(
  ...
  try {
    ...
    if (isExperimentalProvider(conversation.ai_assistant_type)) {
      requestOptions.transcript = await buildExperimentalTranscript(conversation.id);
    }
...
async function handleStreamMode(
  ...
  if (newSessionId && !isExperimentalProvider(conversation.ai_assistant_type)) {
