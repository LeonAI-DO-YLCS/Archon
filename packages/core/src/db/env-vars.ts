...
export async function resolveEnvironmentValue(
  key: string,
  codebaseId?: string
): Promise<string | undefined> {
  if (codebaseId) {
    const codebaseEnvVars = await getCodebaseEnvVars(codebaseId);
    const codebaseValue = codebaseEnvVars[key];
    if (codebaseValue) {
...
export async function resolveLiteLLMApiKey(codebaseId?: string): Promise<string | undefined> {
  return resolveEnvironmentValue('LITELLM_API_KEY', codebaseId);
}
