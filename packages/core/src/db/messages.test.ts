...
import { addMessage, listMessages, listReplayTranscript } from './messages';
...
describe('messages', () => {
  ...
  describe('listReplayTranscript', () => {
    test('returns replayable user and assistant messages in chronological order', async () => {
      ...
      const result = await listReplayTranscript('conv-456', 10);
    ...
    test('filters tool and workflow metadata from replay transcript', async () => {
      mockQuery.mockResolvedValueOnce(
        createQueryResult([
          {
            ...mockMessage,
          ...
          },
          {
            ...
            id: 'msg-2',
            role: 'assistant',
            content: 'Workflow dispatched',
            metadata: '{"category":"workflow_dispatch_status"}',
          },
          {
            ...mockMessage,
            id: 'msg-1',
      ...
      const result = await listReplayTranscript('conv-456');

      expect(result).toEqual([{ role: 'assistant', content: 'Visible answer' }]);
    });
  });
});
