...
/**
 * Update the AI assistant type for an existing codebase.
 * This affects only future conversations; existing conversations retain their assistant type.
 * Throws when the codebase does not exist.
 */
export async function updateCodebaseAssistantType(
  id: string,
  assistantType: string
): Promise<void> {
  const dialect = getDialect();
  const result = await pool.query(
    `UPDATE remote_agent_codebases SET ai_assistant_type = $1, updated_at = ${dialect.now()} WHERE id = $2`,
    [assistantType, id]
  );
  if ((result.rowCount ?? 0) === 0) {
    throw new Error(`Codebase ${id} not found`);
