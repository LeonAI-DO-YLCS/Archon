...
describe('sessions', () => {
  ...
  describe('createSession', () => {
    ...
    test('forces assistant_session_id to null for experimental providers', async () => {
      const experimentalSession: Session = {
        ...mockSession,
        ai_assistant_type: 'ollama',
        assistant_session_id: null,
      };
      mockQuery.mockResolvedValueOnce(createQueryResult([experimentalSession]));

      const result = await createSession({
        conversation_id: 'conv-456',
        ai_assistant_type: 'ollama',
        assistant_session_id: 'should-be-ignored',
      });

      expect(result).toEqual(experimentalSession);
  ...
  describe('usesNativeAssistantSession', () => {
    test('returns true only for claude and codex', () => {
      expect(usesNativeAssistantSession('claude')).toBe(true);
      expect(usesNativeAssistantSession('codex')).toBe(true);
      expect(usesNativeAssistantSession('ollama')).toBe(false);
      expect(usesNativeAssistantSession('lmstudio')).toBe(false);
      expect(usesNativeAssistantSession('litellm')).toBe(false);
    });
  });
