...
`);
  ...
  describe('loadConfig', () => {
    test('returns defaults when no configs exist', async () => {
      ...
      expect(config.assistants.ollama).toEqual({
        enabled: false,
        baseUrl: 'http://localhost:11434',
        defaultModel: 'llama3.2',
      });
      expect(config.assistants.lmstudio).toEqual({
        enabled: false,
        baseUrl: 'http://localhost:1234',
        defaultModel: 'local-model',
      });
      expect(config.assistants.litellm).toEqual({
        enabled: false,
...
`);
    ...
    test('merges experimental provider defaults from global and repo config', async () => {
      const pathMatches = (path: string, pattern: string): boolean => {
        const normalizedPath = path.replace(/\\/g, '/');
        return normalizedPath.includes(pattern);
      };
...
`);
    ...
    test('writes experimental provider config without persisting secrets', async () => {
      mockReadConfigFile.mockResolvedValue('');
      ...
      const writtenContent = mockWriteConfigFile.mock.calls[0]?.[1] as string;
      expect(writtenContent).toContain('litellm');
      expect(writtenContent).toContain('http://litellm.local');
      expect(writtenContent).not.toContain('LITELLM_API_KEY');
    });
...
`);
    ...
    test('includes experimental provider settings in safe config', async () => {
      mockReadConfigFile.mockResolvedValue(`
assistants:
  ollama:
    enabled: true
...
`);
      ...
      expect(safe.assistants.ollama).toEqual({
        enabled: true,
        baseUrl: 'http://ollama.local',
        defaultModel: 'llama3.1',
      });
      expect(safe.assistants.litellm).toEqual({
        enabled: true,
        baseUrl: 'http://litellm.local',
        defaultModel: 'gpt-4o-mini',
      });
      expect(JSON.stringify(safe)).not.toContain('LITELLM_API_KEY');
    });
  });
});
